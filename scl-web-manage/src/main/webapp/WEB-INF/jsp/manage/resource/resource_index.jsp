<%--
  Created by IntelliJ IDEA.
  User: shengchenglong
  Date: 17/2/22
  Time: 16:36
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>资源管理</title>
</head>
<body>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<link rel="stylesheet" href="${ctx}/static/manage/jquery-treetable/stylesheets/jquery.treetable.css" />
<style>
    /* treeTable中父节点的小三角形 */
    table.treetable tr.collapsed span.indenter a { background-image: url(data:image/png;base64,); }
    table.treetable tr.expanded span.indenter a { background-image: url(data:image/png;base64,); }
    .btn { font-size: 11px; padding: 2px 7px; }
</style>

<div class="row">
    <table id="table" class="table  table-bordered table-hover">
        <thead>
            <tr>
                <th>名称</th>
                <th>类型</th>
                <th>URL路径</th>
                <th>权限字符串</th>
                <th>操作</th>
            </tr>
        </thead>
        <tbody>
            <c:if test="${resourceList ne null }">
                <c:forEach items="${resourceList}" var="resource">
                    <!-- 如果不是 根节点 就有父节点， -->
                    <tr data-tt-id='${resource.code}' <c:if test="${not resource.rootNode}">data-tt-parent-id='${resource.parentCode}'</c:if>>
                        <td>${resource.name}</td>
                        <td>${resource.type}</td>
                        <td>${resource.url}</td>
                        <td>${resource.permission}</td>
                        <td>
                                <%-- <shiro:hasPermission name="resource:create"> --%>
                            <!-- 如果不是按钮就是菜单 可以添加子节点 -->
                            <c:if test="${resource.type ne 'button'}">
                                <button class="addBtn btn btn-white btn-success btn-round" resourceCode="${resource.code }">
                                    <i class="ace-icon glyphicon glyphicon-plus"></i>
                                    添加子节点
                                </button>

                            </c:if>
                                <%-- </shiro:hasPermission> --%>

                                <%-- <shiro:hasPermission name="resource:update"> --%>
                                <button class="updateBtn btn btn-white btn-info btn-round" resourceId="${resource.id }">
                                    <i class="ace-icon fa fa-pencil-square-o"></i>
                                    修改
                                </button>
                                <%-- </shiro:hasPermission> --%>


                            <c:if test="${not resource.rootNode}">
                                <%-- <shiro:hasPermission name="resource:delete"> --%>
                                <button id="deleteBtn" class="deleteBtn btn btn-white btn-warning btn-round" resourceId="${resource.id }">
                                    <i class="ace-icon glyphicon glyphicon-trash"></i>
                                    删除
                                </button>

                                <%-- </shiro:hasPermission> --%>
                            </c:if>
                        </td>
                    </tr>
                </c:forEach>
            </c:if>
        </tbody>
    </table>
</div>

<script type="text/javascript" src="${ctx}/static/manage/jquery-treetable/javascripts/src/jquery.treetable.js"></script>
<script type="text/javascript">
    $(function() {
        $("#table").treetable({expandable : true}).treetable("expandNode", "root");

        // 新增资源
        $(".addBtn").click(function() {
            window.location.href = '${ctx}/user/resource/toCreate?parentCode=' + $(this).attr("resourceId") + '&layer=1';
        });

        //更新资源
        $(".updateBtn").click(function() {
            window.location.href = '${ctx}/user/resource/toUpdate?code=' + $(this).attr("code") + '&layer=1';
        });

    });
</script>

</body>
</html>
